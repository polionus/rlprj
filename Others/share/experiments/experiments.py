...
class RewardCallback(BaseCallback):
    ...
    def _on_training_end(self) -> None:
        filename_returns = f"Alg{se}_env{self.env_id}_seed{self.seed}_tmultiplier{self.delta_t}_alpha{self.alpha}_RETURNS.npy"
        filename_rewards = f"Alg{self.model_idlf.model_id}_env{self.env_id}_seed{self.seed}_tmultiplier{self.delta_t}_alpha{self.alpha}_REWARDS.npy"
...
class Experiment:
    ...
    def make_env(self):
        ...
        if self.env_id == "CartPole":
            ...
            max_episode_steps = 1000/self.delta_t, 
             )
